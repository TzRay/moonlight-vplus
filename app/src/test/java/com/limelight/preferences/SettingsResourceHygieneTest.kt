...
class SettingsResourceHygieneTest {
    ...
    @Test
    fun manualHdrSettingsKeepUniqueKeysAndDecimalInput() {
        val preferences = parse(File(resourceDir, "xml/preferences.xml"))
        val elements = preferences.documentElement.getElementsByTagName("*")
            .asElementSequence().toList()
        val keys = elements.map { it.getAttributeNS(ANDROID_NAMESPACE, "key") }
            .filter(String::isNotEmpty)
        assertTrue("Preference keys must be unique after merging", keys.size == keys.toSet().size)
        ...
        listOf(
            ...
            assertTrue("Missing manual HDR setting: $key", key in keys)
        }
        // 最低亮度必须允许输入小数，避免合并后丢失 0.001 nits 精度。
        val minimum = elements.single {
            it.getAttributeNS(ANDROID_NAMESPACE, "key") == "edittext_hdr_manual_min_brightness"
        }
        assertTrue(minimum.getAttributeNS(ANDROID_NAMESPACE, "inputType") == "numberDecimal")
    }
